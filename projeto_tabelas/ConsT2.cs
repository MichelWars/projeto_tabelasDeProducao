...
{
    public partial class ConsT2 : Form
    {
        ...
        private void btnConsTodos_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            String sql = "Select data, equipamento,horario, meta_unidades, acumulado_unidades, parada_minutos, observacao from periodo_tarde order by equipamento,data";
            String sql2 = "Select * from relatorio_tarde";
        ...
        private void ConsData(object sender, EventArgs e)
        {
            conn = new MySqlConnection("server=localhost;uid=root; pwd='';database=prada;useAffectedRows=true");
            conn.Open();
            String sql = "Select data, equipamento,horario, meta_unidades, acumulado_unidades, parada_minutos, observacao from periodo_tarde where data like @data";
            String sql2 = "Select * from relatorio_manha where data like @data";
            cmd = new MySqlCommand(sql, conn);
            cmd2 = new MySqlCommand(sql2, conn);
        ...
        private void ConsEq(object sender, EventArgs e)
        {
            conn = new MySqlConnection("server=localhost;uid=root; pwd='';database=prada;useAffectedRows=true");
            conn.Open();
            String sql = "Select data, equipamento,horario, meta_unidades, acumulado_unidades, parada_minutos, observacao from periodo_tarde where equipamento = @equipamento";
            String sql2 = "Select * from relatorio_tarde where equipamento = @equipamento";
            cmd = new MySqlCommand(sql, conn);
            cmd2 = new MySqlCommand(sql2, conn);
        ...
        private void ajustarTamanho2(object sender, EventArgs e)
        {
            int larguraTela = this.Size.Width;
            int alturaTela = this.Size.Height;
            int larguraConsT2 = groupConst2.Size.Width;
            int alturaConsT2 = groupConst2.Size.Height;

            Point p = new Point();
            p.X = (larguraTela - larguraConsT2) / 2;
            p.Y = (alturaTela - alturaConsT2) / 2;
            groupConst2.Location = p;
        }
