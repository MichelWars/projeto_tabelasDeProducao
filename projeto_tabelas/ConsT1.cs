...
namespace projeto_tabelas
{
    public partial class ConsT1 : Form
    {
        ...
        private void btnConsTodos_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            String sql = "Select data, equipamento,horario, meta_unidades, acumulado_unidades, parada_minutos, observacao from periodo_manha order by equipamento,data";
            String sql2 = "Select * from relatorio_manha";
        ...
        private void ConsData(object sender, EventArgs e)
        {
            conn = new MySqlConnection("server=localhost;uid=root; pwd='';database=prada;useAffectedRows=true");
            conn.Open();
            String sql = "Select data, equipamento,horario, meta_unidades, acumulado_unidades, parada_minutos, observacao from periodo_manha where data like @data";
        ...
        private void ConsEq(object sender, EventArgs e)
        {
            conn = new MySqlConnection("server=localhost;uid=root; pwd='';database=prada;useAffectedRows=true");
            conn.Open();
            String sql = "Select data, equipamento,horario, meta_unidades, acumulado_unidades, parada_minutos, observacao from periodo_manha where equipamento = @equipamento";
            String sql2 = "Select * from relatorio_manha where equipamento = @equipamento";
            cmd = new MySqlCommand(sql, conn);
            cmd2 = new MySqlCommand(sql2, conn);
        ...
        private void ajustarTamanho(object sender, EventArgs e)
        {
            ...
            int larguraConsT1 = groupConst1.Size.Width;
            int alturaConsT1 = groupConst1.Size.Height;

            Point p = new Point();
            p.X = (larguraTela - larguraConsT1) / 2;
            p.Y = (alturaTela - alturaConsT1) / 2;
            groupConst1.Location = p;
        }
